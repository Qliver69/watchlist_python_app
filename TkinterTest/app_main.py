...
class App:
    ...
    def __init__(self):
        ...
        self.frm_btnBD = tk.Frame(self.frm_body, bg=self.colorBG)  # frame middle left
        ...
        self.lbl_animeName = tk.Label(self.frm_label_name, text="Anime's Name",
                                      bg=self.colorBG, font=('arial', 18), width=33, anchor=tk.CENTER)
    ...
    def config_window(self):
        ...
        self.window.title('WatchList TC')
        self.window.iconbitmap('data\\file.ico')
...
# set all left side of the app
    def set_body(self, evt):
        ...
        if self.anime_list[self.find_index()][3] == '1':
            self.ckbtn_finish.select()
        else:
            self.ckbtn_finish.deselect()

# find te good index for an anime on the list
    def find_index(self):
        x = 0
        # run while the text selected on the list is different tha the name of the anime
        while self.listboxWL.get(self.listboxWL.curselection()) != self.anime_list[x][0]:
            x += 1
        return x

    def save(self):
        # delete then create the anime file
        os.remove('data\\Anime.txt')
        file = open('data\\Anime.txt', 'w+')
        for i in range(len(self.anime_list)):
            for j in range(3):
                # put every element on one string
        ...
        file.close()
        # coming back to first page
        self.set_anime_list()

    def checkbox_click(self):
        if self.anime_list[self.find_index()][3] == '0':
            self.anime_list[self.find_index()][3] = '1'
        else:
            self.anime_list[self.find_index()][3] = '0'

# create a new process to run the selenium1.py file on new console
